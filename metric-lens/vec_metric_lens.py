...
def metric_skill(raw: float | None, spec: Dict[str, Any]) -> float:
    ...
    if direction == "higher":
        d = ceil - m; d_floor = ceil - floor
    else:
        d = m - ceil; d_floor = floor - ceil
    if d <= 0: return 1.0
    if d_floor <= 0: raise ValueError(f"Invalid anchors for {spec}")
    return min(d_floor / (d_floor + d), 1.0)

def board_score(metrics: Dict[str, Any], board: str) -> Dict[str, Any]:
    spec = BOARDS[board]["metrics"]; rows=[]; total=0.0
    for name, mspec in spec.items():
        raw=metrics.get(name); skill=metric_skill(raw, mspec); contribution=100.0*mspec["weight"]*skill; total+=contribution
        rows.append({"metric":name,"label":mspec["label"],"raw":raw,"skill":skill,"weight":mspec["weight"],"points":contribution,"max_points":100.0*mspec["weight"]})
    return {"board":board,"score":total,"rows":rows}

def _raw_at_fraction_to_ceiling(raw, spec, frac):
    if raw is None or not isinstance(raw,(int,float)) or not math.isfinite(float(raw)):
        return float(spec["ceiling"])
    x=float(raw)
    if spec["direction"] != "abs_lower": return x + frac*(float(spec["ceiling"])-x)
    cur_abs=abs(x); ceil_abs=abs(float(spec["ceiling"])); new_abs=cur_abs+frac*(ceil_abs-cur_abs)
    sign=-1.0 if x<0 else (1.0 if x>0 else (-1.0 if float(spec["ceiling"])<0 else 1.0))
    return sign*new_abs

def counterfactuals(metrics, board, frac=1.0):
    base=board_score(metrics,board)["score"]; out=[]
    for name,mspec in BOARDS[board]["metrics"].items():
        alt=deepcopy(metrics); alt[name]=_raw_at_fraction_to_ceiling(metrics.get(name),mspec,frac); s=board_score(alt,board)["score"]
...
def load_metrics(path: Path) -> Dict[str, Any]:
    ...
    if "metrics" in obj and isinstance(obj["metrics"],dict): return obj["metrics"]
    if isinstance(obj,dict): return obj
    raise ValueError("Expected a JSON object or a veckit result with a top-level 'metrics' object")

def render_markdown(metrics, board):
    result=board_score(metrics,board); gains=counterfactuals(metrics,board,frac=1.0)
    lines=[f"# VEC Metric Lens — `{board}`","",f"**Reconstructed public validation score: {result['score']:.2f} / 100**","","> Uses the organizers' published validation anchors and task weights. This is an explanation of a known local/validation metric vector, not a predictor of the hidden test score.","","## Score breakdown","","| Metric | Raw | Skill | Weight | Points | Points left |","|---|---:|---:|---:|---:|---:|"]
    ...
    for i,g in enumerate(gains,1): lines.append(f"| {i} | {g['metric']} | {g['new_score']:.2f} | +{g['gain']:.2f} |")
    lines += ["","## A few cautions","","- A floor-like model is around 50 because each metric's floor maps to skill 0.5.","- The ceiling is an empirical split-half estimate, not a theoretical maximum; beating it clips that metric's skill to 1.","- `scale_log_ratio` and `severity_slope` are scored by absolute value before the lower-is-better transform.","- Missing/NaN scored metrics count as skill 0 under the published rules.","- Improving a public-validation bottleneck can still fail to transfer to the hidden test target."]
    return "\n".join(lines)+"\n"

def main():
    ap=argparse.ArgumentParser(); ap.add_argument("input",type=Path,help="veckit result JSON or a bare metrics JSON object"); ap.add_argument("--board",required=True,choices=sorted(BOARDS)); ap.add_argument("--markdown",type=Path); ap.add_argument("--json",dest="json_out",type=Path); args=ap.parse_args()
    metrics=load_metrics(args.input); result=board_score(metrics,args.board); result["counterfactual_ceiling"]=counterfactuals(metrics,args.board,1.0); text=render_markdown(metrics,args.board); print(text)
    if args.markdown: args.markdown.write_text(text)
